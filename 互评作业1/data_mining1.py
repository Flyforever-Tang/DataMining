...
class MissingProcessing(object):
    ...
    def frequencyFill(data_array: np.ndarray) -> np.ndarray:
        ...
        return SimpleImputer(strategy='most_frequent').fit_transform(data_array)
    ...
    def relevanceFill(data_array: np.ndarray) -> np.ndarray:
        if len(data_array.shape) == 1:
            data_array = data_array.reshape(-1, 1)
        return IterativeImputer().fit_transform(data_array)
...
def dataSummary(data_frame: DataFrame,
    ...
    def statistics(data_array: np.ndarray) -> Tuple[float, float, float, float, float, int, list]:
        ...
        return \
            describe['min'], describe['25%'], describe['50%'], describe['75%'], describe['max'], \
            sum(1 for data in data_array if np.isnan(data)), missing_method(data_array)
    ...
    for i in range(len(data_frame.columns)):
        ...
        if attribute_types[i] == AttributeType.Nominal:
            ...
            print(i, list(summary_results[-1].values()).count(1), len(values), max(summary_results[-1].values()))
...
def visualize(summary_results: List[Union[dict, Tuple, None]],
    ...
    for summary_result, attribute_type, attribute_name in zip(summary_results, attribute_types, attribute_names):
        ...
        if attribute_type == AttributeType.Numeric:
            ...
            fig_index += 1
            plt.subplot(fig_y_sum, fig_x_sum, fig_index)
            plt.title(f'Boxplot of {attribute_name}(with outliers)')
            plt.boxplot(summary_result[-1], showfliers=True)
            fig_index += 1
            plt.subplot(fig_y_sum, fig_x_sum, fig_index)
            plt.title(f'Boxplot of {attribute_name}(without outliers)')
...
def processing(csv_file_path: str,
               attribute_types: List[AttributeType],
               missing_method: Callable[[np.ndarray], Union[list, np.ndarray]] = MissingProcessing.eliminate) -> None:
...
csv_file_path = '../data/Wine Reviews/winemag-data_first150k.csv'
...
attributeType = [AttributeType.Nonsense if i in nonsense_columns
                 ...
                 for i in range(11)]
# 依次采用不同的缺失数据填充方法进行处理，默认先使用直接剔除nan值的方法
processing(csv_file_path, attributeType)
...
csv_file_path = '../data/Wine Reviews/winemag-data-130k-v2.csv'
# 标注出无关属性、数值属性的列序号，其余的为标称属性
...
csv_file_path = '../data/Chicago Building Violations/building-violations.csv'
# 标注出无关属性、数值属性的列序号，其余的为标称属性
